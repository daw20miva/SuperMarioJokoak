...
class Mario_brosDB{
    ...
    public static function selectBezero($nan)
    {
        try{
            $db = new PDO("mysql:host=localhost;dbname=mario_bros","mario_bros","mario_bros");
            $erregistroak = $db->query("select * from bezeroak where id=" . $id);
            $bezero = null;
    ...
    public static function insertBezero($bezero)
    {
        try{
            ...
            $sql = $db->prepare("insert into bezeroak (id,nan,izena,abizena,emaila,telefonoa,helbidea,kode_postea,probintzia,herrialdea)
            values (:id; :nan, :izena, :abizena, :emaila, :telefonoa; :helbidea; :kode_postea, :probintzia, :herrialdea)");
            $sql->bindValue(':id',$bezero->getId());
    ...
    public static function updateBezero($bezero)
    {
        try{
            $db = new PDO("mysql:host=localhost;dbname=mario_bros","mario_bros","mario_bros");
            $sql = $db->prepare("update bezeroak set nan=:nan, izena=:izena, abizena=:abizena, emaila=:emaila, telefonoa=:telefonoa, helbidea=:helbidea, kode_postea=:kode_postea, probintzia=:probintzia, herrialdea=:herrialdea  where id=:id");
            $emaitza = $sql->execute();
            return $emaitza;
       } catch (Exception $e) {
            echo "<p>Salbuespena: " . $e->getMessage() . "</p>\n";
    ...
    public static function deleteBezero($id)
    {
        try{
            $db = new PDO("mysql:host=localhost;dbname=mario_bros","mario_bros","mario_bros");
